...
class PenjualanDetailController extends Controller
{
    ...
    {
        $obat = DetailObat::with(['obat'])->where('cabang_id', auth()->user()->cabang_id)->where('stock','!=',0)->orderBy('produk_id','ASC')->get();
    ...
    {  $detail = PenjualanDetail::with(['prodk','obat'])
        ...
        foreach ($detail as $item) {
            ...
            $row['ed'] =$item->obat['ed'] ?? 0;
            $row['harga_jual']  = 'Rp. ' . format_uang($item->harga_jual);
            $row['jumlah']      = '<input type="number" data-max="'.$item->obat['stock'].'" class="form-control input-sm quantity" data-id="' . $item->id . '" value="' . $item->jumlah . '" style="width: 100px; ">';
            $row['diskon']      = '<p style="width:20px;">'. $item->diskon . '%'.'</p>';
            $row['subtotal']    = 'Rp. ' . format_uang($item->subtotal);
            $row['aksi']        = '<div class="btn-group" style="width: 20px;">
        ...
        return datatables()
            ...
            ->rawColumns(['aksi', 'kode_produk', 'jumlah','ed','nama_produk','harga_jual','subtotal','diskon'])
            ->make(true);
    }

    public function store(Request $request)
    {
        $obat = DetailObat::where('id', $request->produk_id)->first();
        $produk = Produk::where('id', $obat->produk_id)->first();
        if (!$obat) {
            # code...
        ...
        try {
            ...
            $detail->harga_beli = $obat->harga_beli;
            $detail->harga_jual = $obat->harga_jual;
            $detail->jumlah = 1;
            $detail->diskon = $obat->diskon;
            $detail->subtotal = $obat->harga_jual - ($obat->diskon / 100 * $obat->harga_jual);
            $detail->laba = $obat->harga_beli - $obat->harga_jual - ($obat->diskon / 100 * $obat->harga_jual);
            $detail->save();
